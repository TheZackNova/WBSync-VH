    ...
    books.forEach(book => {
      ...
      div.innerHTML = `
        ...
        <div class="wb-sync-manage-wb-actions">
          <button class="wb-sync-button wb-sync-btn-small wb-sync-manage-wb-download">⬇️ 下载</button>
...
function renderEntriesList(entries) {
  ...
  entries.forEach(entry => {
    ...
    const entryName = entry.name || entry.comment || (entry.key && entry.key.length > 0 ? entry.key.join(', ') : '未命名条目');
    const div = document.createElement('div');
